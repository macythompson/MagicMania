...
void setup() {
  ...
  backgrounds[0] = new BackGround(300, 300, "Home", "Welcome to the game! Your mother is sick and you need to save her!", "Talk to your mother? A - Yes B - No", true, false);
  backgrounds[1] = new BackGround(300, 300, "outside", "find the things you need.", "REEEEEEEEEEEEEEEEEEE", true, false);
...
void keyPressed() {
  if (key == CODED) {
    if (keyCode == RETURN) {
      if (backgrounds[0].dialogueOne) {
        fill(120);
        text(backgrounds[0].dialogue1, 600, 600);
      } else if (backgrounds[0].dialogueOne == false) {
        fill(120);
        text(backgrounds[0].dialogue2, 600, 600);
      } 
    } 
